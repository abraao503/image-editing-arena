...
const App: React.FC = () => {
  ...
  // Input Images (starts empty, supports multiple)
  ...
  const handleRun = async () => {
    ...
    const isImageMissing = inputImages.length === 0;
    ...
    setResults(
      selectedModels.map((id) => ({ modelId: id, status: "starting" })),
    );
    ...
    const promises = selectedModels.map(async (modelId) => {
      ...
      try {
        setResults((prev) =>
          prev.map((r) =>
            r.modelId === modelId ? { ...r, status: "processing" } : r,
          ),
        );
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#F9FAFB] dark:bg-[#0a0a0a] text-gray-900 dark:text-gray-100 font-sans overflow-hidden">
      ...
      >
        ...
        <div className="hidden md:flex px-5 py-6 border-b border-gray-100 dark:border-[#222] items-center justify-between shrink-0 transition-colors">
          <div className="flex flex-col gap-1.5 items-start">
            ...
            <svg
              ...
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 w-auto text-black dark:text-white transition-colors"
            >
              <path
                d="M197.3 64.2C220.5 64.2 240 79.6 240 108.2C240 110.1 240 111.7 239.8 114.6H170.1C171 129.1 182.7 138.7 197.4 138.7C209.8 138.7 218.1 133 222.7 125.2L237.6 135.8C229.6 148.4 215.8 156.4 197.2 156.4C170.3 156.4 150.9 137.3 150.9 110.3C151 84.2 170.4 64.2 197.3 64.2ZM171.1 100.3H220.4C218.3 87.5 208 80.6 196.6 80.6C185.2 80.6 173.7 87.2 171.1 100.3Z"
                fill="currentColor"
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col">
          {/* 1. IMAGE UPLOAD - supports multiple images */}
          <div
            onDragOver={handleDragOver}
            onDragLeave={handleDragLeave}
            onDrop={handleDrop}
            className={`relative w-full bg-gray-50 dark:bg-[#121212] overflow-hidden transition-all border-b border-gray-100 dark:border-[#222] shrink-0
                  ${validationError && inputImages.length === 0 ? "ring-2 ring-red-500 ring-inset z-10" : ""}
                  ${isDragging ? "ring-4 ring-blue-500 ring-inset bg-blue-50 dark:bg-blue-950/20" : ""}`}
          >
            {inputImages.length === 0 ? (
              <div
                onClick={() => fileInputRef.current?.click()}
                className={`aspect-square cursor-pointer flex flex-col items-center justify-center transition-colors hover:bg-gray-100 dark:hover:bg-[#181818] ${isDragging ? "text-blue-500 dark:text-blue-400" : "text-gray-400 dark:text-gray-500"}`}
              >
                <Upload
                  size={isDragging ? 32 : 24}
                  className={`mb-2 transition-all ${validationError && inputImages.length === 0 ? "text-red-500" : isDragging ? "text-blue-500 animate-bounce" : "opacity-50"}`}
                />
                <span
                  className={`text-xs font-medium uppercase tracking-wider text-center px-4 ${validationError && inputImages.length === 0 ? "text-red-500" : isDragging ? "text-blue-500 dark:text-blue-400" : ""}`}
                >
                  {validationError && inputImages.length === 0
                    ? "Image Required"
                    : isDragging
                      ? "Drop Images Here"
              ...
              <div className="p-3">
                <div className="grid grid-cols-3 gap-2">
                  {inputImages.map((img, index) => (
                    <div
                      key={index}
                      className="relative group aspect-square bg-gray-100 dark:bg-[#1a1a1a] overflow-hidden rounded-md"
                    >
                      <img
                        src={img}
                        alt={`Input ${index + 1}`}
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 bg-black/50 hover:bg-black/70 text-white p-1 backdrop-blur-sm transition-all opacity-100 md:opacity-0 md:group-hover:opacity-100 rounded touch-manipulation active:scale-95"
                      >
                        <Trash2 size={12} />
                      </button>
                    </div>
                  ...
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="aspect-square flex flex-col items-center justify-center gap-1 border border-dashed border-gray-300 dark:border-[#333] rounded-md text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-[#181818] hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
                  >
                    <Upload size={18} />
                ...
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-2 px-0.5">
                  {inputImages.length} image
                  {inputImages.length > 1 ? "s" : ""} · single-image models use
                  the first
                </p>
              </div>
            )}
            ...
            {isDragging && inputImages.length > 0 && (
          ...
          <div
            className={`bg-white dark:bg-[#0f0f0f] border-b shrink-0 ${validationError && !prompt.trim() ? "border-red-500 ring-1 ring-red-500 z-10" : "border-gray-100 dark:border-[#222]"}`}
          >
            <textarea
              ...
              placeholder={
                validationError && !prompt.trim()
                  ? "A prompt is required to run the models..."
          ...
          <div className="bg-white dark:bg-[#0f0f0f] border-b border-gray-100 dark:border-[#222] shrink-0 px-5 py-4">
            ...
            <input
              ...
              placeholder="owner/model or .safetensors URL"
              className="w-full bg-gray-50 dark:bg-[#1a1a1a] border border-gray-100 dark:border-[#333] rounded px-3 py-2 text-xs text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-600 outline-none focus:border-gray-300 dark:focus:border-[#555] transition-colors"
            />
          </div>
          ...
          {results.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center opacity-50">
              ...
              <p className="text-gray-400 dark:text-gray-600 text-sm">
                Select an image and run models to see results
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 pb-20">
              {results
                .sort((a, b) => {
                  const indexA = AVAILABLE_MODELS.findIndex(
                    (m) => m.id === a.modelId,
                  );
                  const indexB = AVAILABLE_MODELS.findIndex(
                    (m) => m.id === b.modelId,
                  );
                ...
                .map((result) => {
                  ...
                  return (
                    <div
                      key={result.modelId}
                      className="group flex flex-col bg-white dark:bg-[#0f0f0f] shadow-sm hover:shadow-lg transition-all duration-300 aspect-[6/7] border border-transparent dark:border-[#222]"
                    >
